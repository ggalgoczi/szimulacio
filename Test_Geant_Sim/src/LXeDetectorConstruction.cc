...
void LXeDetectorConstruction::DefineMaterials(){
  ...
  G4double density_CsI;
	...
	a = 132.90543*g/mole;
	G4Element* elCs  = new G4Element("Cesium"  ,"C" , z= 55., a);
  ...
  //Pb
  fPb = new G4Material("Pb",z=82.,a=207.2*g/mole,density=11.35*g/cm3);
 ...
 // G4double lxe_RIND[lxenum]  = { 2.1 , 2.04 , 1.96 ,1.9 ,1.83 ,1.79 ,1.7 ,1.68};
  G4double lxe_ABSL[lxenum]  = { 50.*cm, 50.*cm, 50.*cm, 50.*cm, 50.*cm, 50.*cm, 50.*cm, 50.*cm};
  CsI_mt = new G4MaterialPropertiesTable();
  CsI_mt->AddProperty("SLOWCOMPONENT", lxe_Energy, lxe_SCINT, lxenum);
  CsI_mt->AddProperty("RINDEX",        lxe_Energy, lxe_RIND,  lxenum);
  CsI_mt->AddProperty("ABSLENGTH",     lxe_Energy, lxe_ABSL,  lxenum);
  CsI_mt->AddConstProperty("SCINTILLATIONYIELD",540./keV);
  ...
  fLXe->SetMaterialPropertiesTable(CsI_mt);
...
//			}
  ...
  Pb1_log = new G4LogicalVolume(Pb1_box_With_Hole, fPb, "Pb1_log",0,0,0);
...
//  G4SubtractionSolid Pb2_box_With_Hole("Box-Cylinder", &Pb2_box, &Cylinder1);
  ...
  Pb2_log = new G4LogicalVolume(Pb1_box_With_Hole, fPb, "Pb1_log",0,0,0);
...
void LXeDetectorConstruction::SetDefaults() {
  ...
  if(CsI_mt)CsI_mt->AddConstProperty("SCINTILLATIONYIELD",5400./keV);
...
void LXeDetectorConstruction::SetMainScintYield(G4double y) {
  CsI_mt->AddConstProperty("SCINTILLATIONYIELD",y/keV);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
 
void LXeDetectorConstruction::SetWLSScintYield(G4double y) {
  fMPTPStyrene->AddConstProperty("SCINTILLATIONYIELD",y/keV);
}
