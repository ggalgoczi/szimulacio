...
//
  ...
  myMPT1->AddConstProperty("SCINTILLATIONYIELD",0.00001/MeV);
...
//Scint1
  ...
  G4Box* scint = new G4Box("scint",tw-0.01*mm,th-0.01*mm,tl-0.01*mm);
  G4LogicalVolume* scint_log = new G4LogicalVolume(scint,water,"scint",0,0,0);

  G4VPhysicalVolume* scint_phys[100];

  for (int i=0;i<100;i++) {
     G4double x_shift = i*(2*tw)*mm;
     scint_phys[i] = new G4PVPlacement(0,G4ThreeVector(x_shift,0.,0. ),scint_log,
                      "scint",expHall_log,
...
//Scint2
  ...
  G4Box* scint2 = new G4Box("scint2",tw-0.01*mm,th-0.01*mm,tl-0.01*mm);
  G4LogicalVolume* scint2_log = new G4LogicalVolume(scint2,water,"scint2",0,0,0);

// G4VPhysicalVolume* scint2_phys = new G4PVPlacement(myRotation,G4ThreeVector(500*mm,500*mm,-11*mm),scint2_log,"scint2",expHall_log,false,0);

  G4VPhysicalVolume* scint2_phys[100];

   for (int i=0;i<100;i++) {
     G4double y_shift = 497.5*mm-i*(2*tw)*mm;
     scint2_phys[i] = new G4PVPlacement(myRotation,G4ThreeVector(497.5*mm,y_shift,11*mm ),scint2_log,
                      "scint2",expHall_log,
                      false,i,checkOverlaps );
     }
...
/*  G4Box* up = new G4Box("up",tw*mm,tw*mm,tw*mm);
  G4LogicalVolume* up_log = new G4LogicalVolume(up,water,"up",0,0,0);

  G4VPhysicalVolume* up_phys[100];

  for (int i=0;i<100;i++) {
     G4double x_shift = i*(2*tw+1)*mm;
     up_phys[i] = new G4PVPlacement(0,G4ThreeVector(x_shift,505*mm,0.),up_log,
                      "up",expHall_log,
                      false,i,checkOverlaps );
  }
...
//checkOverlaps   
  ...
  G4Box* down = new G4Box("down",tw*mm,tw*mm,tw*mm);
  G4LogicalVolume* down_log = new G4LogicalVolume(down,water,"down",0,0,0);

  G4VPhysicalVolume* down_phys[100];

  for (int i=0;i<100;i++) {
     G4double x_shift = i*(2*tw+1)*mm;
     down_phys[i] = new G4PVPlacement(0,G4ThreeVector(x_shift,-505*mm,0.),down_log,
                      "down",expHall_log,
                      false,i,checkOverlaps );
  }
...
//
  ...
  opWaterSurfaceS->SetFinish(polished);
  opWaterSurfaceS->SetModel(unified);

   for (int i=0;i<100;i++) {
     new G4LogicalBorderSurface("WaterSurfaceS",
                                 scint_phys[i],expHall_phys,opWaterSurfaceS);
   }
