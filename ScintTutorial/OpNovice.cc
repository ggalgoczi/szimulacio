...
{
 G4int nThreads = 1;
  G4double tw = 1.0*atoi(argv[1]);
  G4double th = 1.0*atoi(argv[2]);
  G4double tl = 1.0*atoi(argv[3]);
  int NoE=0;
  if (argc==5) NoE=atoi(argv[4]);
  // Evaluate arguments
  //
/*
  if ( argc > 9 ) {
...
*/
  ...
  runManager-> SetUserInitialization(new OpNoviceDetectorConstruction(tw,th,tl));
  // Physics list
