...
//			cout << ss;
    ...
    in.close();   
	in.open ("/home/galgoczi/cubesat/cosmic_spectras/500km_electrons_min.txt", std::fstream::in);
...
double energy = 0;
	...
	if(i == 0){
		if(double(ProtonEnergyMax[0][3]) > number)
			{
			energy = ProtonEnergyMax[i][0];
     //   cout << "ooo" << ElectronEnergyMin[i][3] << " " << number << "\n";
			}			
		}
	else{
	if(double(ProtonEnergyMax[i][3]) > number && double(ProtonEnergyMax[i-1][3]) < number)
			{
			energy = ProtonEnergyMax[i][0];
     //   cout << "ooo" << ElectronEnergyMin[i][3] << " " << number << "\n";
			}		
