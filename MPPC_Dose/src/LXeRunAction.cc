...
// check if file exists and particle names are OK
    ...
    std::ifstream f99("thefileforgun.txt");

	if(!f99.good()){
		G4cout << "*******************************" << G4endl;
		G4cout << "Abort!!!" << G4endl;
		G4cout << "File: " << "thefileforgun.txt" << " does not exist!" << G4endl;
		G4cout << "*******************************" << G4endl;
...
while (std::getline(file2, infileline))
	...
	FileReader* ReadIn = new FileReader(infilename);
...
void LXeRunAction::EndOfRunAction(const G4Run* aRun){
	...
	G4cout << fRun->Edep/eV << G4endl;		
	if (isMaster){	
		G4cout << fRun->Edep << G4endl;	
