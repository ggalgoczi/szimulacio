...
: fLXe_mt(NULL), fMPTPStyrene(NULL)
{
  ...
  fMPPC = ADCS_mat = ANT_mat = COM_mat = EPS_mat = OBC_mat = STRU_mat = SP_mat = Payload_mat = NULL;
...
//  fMg = new G4Element("Mg",z=25.,a=54.9*g/mole,density=7.21*g/cm3); 
 ...
 // Pb = new G4Material("Air", density= 1.29*mg/cm3, 1);
  //fAir->AddElement(fPb, 100*perCent);
 //Aluminum
...
// CAD MESH END	
  ...
  G4double Pb_Thickness = 0.5*mm;

  // Pb shielding
  
  G4double Pb_x = 80 *mm + 2*Pb_Thickness;
  G4double Pb_y = 8*mm + 2*Pb_Thickness;
  G4double Pb_z = 10*mm + Pb_Thickness;
  Pb_box = new G4Box("Pb_box",Pb_x/2., Pb_y/2., Pb_z/2.);
  Pb_log = new G4LogicalVolume(Pb_box,Mat_Pb,"Pb_log",0,0,0);
  Pb_phys = new G4PVPlacement(0,G4ThreeVector(),
                              Pb_log,"Pb_vol",fExperimentalHall_log,0,false,0);                                                 

  // Cut_Out_From_Pb

  G4double CutOut_x = Pb_x - 2*Pb_Thickness;
  G4double CutOut_y = Pb_y - 2*Pb_Thickness;
  G4double CutOut_z = Pb_z - Pb_Thickness;
  CutOut_box = new G4Box("CutOut_box",CutOut_x/2., CutOut_y/2., CutOut_z/2.);
  CutOut_log = new G4LogicalVolume(CutOut_box,  fVacuum,"CutOut_log",0,0,0);
  CutOut_phys = new G4PVPlacement(0,G4ThreeVector(0.,0.,-Pb_Thickness/2.),
                              CutOut_log,"CutOut_vol",Pb_log,0,false,0); 

  //CutOut_log->SetVisAttributes(G4VisAttributes::Invisible);

  // CsI1
  // Overlap of CsI and Pb shielding is 3 mm

  G4double CsI_x1 = CutOut_x;
  G4double CsI_y1 = 5*mm;
  G4double CsI_z1 = 3*mm;
  CsI_box1 = new G4Box("CutOut_box",CsI_x1/2., CsI_y1/2., CsI_z1/2.);
  CsI_log1 = new G4LogicalVolume(CsI_box1,  fLXe,"CsI_log",0,0,0);
  CsI_phys1 = new G4PVPlacement(0,G4ThreeVector(0.,0.,-CutOut_z/2.+CsI_z1/2.),
                              CsI_log1,"CutOut_vol",CutOut_log,0,false,0);   
  
    // CsI2
  
  G4double CsI_x2 = CutOut_x;
  G4double CsI_y2 = 5*mm;
  G4double CsI_z2 = 15*cm-CsI_z1;
  CsI_box2 = new G4Box("CutOut_box",CsI_x2/2., CsI_y2/2., CsI_z2/2.);
  CsI_log2 = new G4LogicalVolume(CsI_box2,  fLXe,"CsI_log2",0,0,0);
  CsI_phys2 = new G4PVPlacement(0,G4ThreeVector(0.,0.,-Pb_z/2.-CsI_z2/2.),
                              CsI_log2,"CutOut_vol",fExperimentalHall_log,0,false,0);
  ...
  MPPC_phys = new G4PVPlacement(0,G4ThreeVector(0,0,-CutOut_z/2.+CsI_z1+MPPC_z/2.),
                              MPPC_log,"MPPC",CutOut_log,0,false,0); 
...
//		SetSensitiveDetector("cad_logical", aTrackerSD, true);
		...
		LXeScintSD* MPPCSD = new LXeScintSD("MPPCSD");
