...
: fLXe_mt(NULL), fMPTPStyrene(NULL)
{
  ...
  Aluminum6061_T6 = Aluminum7075_T73 = StainlessSteelA2_70AISI304 = CopperElectricEN2_0060 = GlassBorosilicateN_BK7 = FR4Glass_Epoxysheet = BrassGeneric = SolarPanel = NULL;
...
std::string filename;	
std::string directoryname("../../RADCUBE_model/");
std::stringstream ss;
ss << directoryname << "ADCS.STL";
filename = ss.str();
...
ss << directoryname << "AUX_backplane.STL";
filename = ss.str();
...
//offset = G4ThreeVector(0*m, 0*m, 0*m);
//offset = G4ThreeVector(-0.17025*m, -0.050000001*m, -0.050000001*m);
CADMesh * mesh9 = new CADMesh(filename, mm, offset, false);
...
cad_logical_9 = new G4LogicalVolume(cad_solid_9, STRU_mat, "cad_logical", 0, 0, 0);
	...
	G4Material* AUX_mat = new G4Material(name="AUX_mat", density, ncomponents=1);
